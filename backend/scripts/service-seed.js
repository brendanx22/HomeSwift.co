...
if (!supabaseUrl || !supabaseServiceKey) {
  console.error('Missing Supabase configuration. Need SUPABASE_SERVICE_ROLE_KEY');
  process.exit(1);
}
...
// Create test landlords using service role key to bypass RLS
const createTestLandlordsDirectly = async () => {
  console.log('👥 Creating test landlords using service role key...');

  const createdLandlordIds = [];

  try {
    // First create auth users using admin API
    for (const landlord of TEST_LANDLORDS) {
      console.log(`👤 Creating auth user: ${landlord.email}`);
      ...
      if (authError) {
        if (authError.message.includes('already exists') || authError.code === 'email_exists') {
          console.log(`⚠️  Auth user ${landlord.email} already exists, skipping...`);
          createdLandlordIds.push(landlord.id);
          continue;
        }
        throw authError;
      }
    ...
    // Now create the profiles (they should already exist due to the trigger, but let's make sure)
    ...
    if (error) {
      ...
      // If profiles already exist, try to fetch them instead
      console.log('🔄 Attempting to fetch existing landlords...');
      const { data: existingLandlords, error: fetchError } = await supabase
        .from('user_profiles')
        .select('id')
        .in('email', TEST_LANDLORDS.map(l => l.email));

      if (fetchError) {
        console.error('❌ Error fetching existing landlords:', fetchError);
        return [];
      }

      console.log(`✅ Found ${existingLandlords.length} existing landlords`);
      return existingLandlords.map(l => l.id);
    }
...
const seedDatabase = async () => {
  console.log('🚀 Starting service role database seeding process...');
  console.log('📋 This will create test landlords and hundreds of properties using service role key');

  // Create test landlords using service role key
  ...
  console.log('');
  console.log('⚠️  Note: These are test accounts. In production, users should sign up normally.');
};
...
if (require.main === module) {
  seedDatabase()
    .then(() => {
      console.log('✅ Service role seeding script completed');
      process.exit(0);
    })
    .catch((error) => {
