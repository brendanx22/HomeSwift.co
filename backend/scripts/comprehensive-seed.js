...
const createTestLandlord = async (landlordData) => {
  try {
    console.log(`👤 Creating landlord: ${landlordData.email}`);
    ...
    const { data: authData, error: authError } = await supabase.auth.signUp({
      ...
      options: {
        data: {
          ...
          agent_id: landlordData.agent_id
        }
      }
    });
    ...
    if (authError) {
      if (authError.message.includes('already registered')) {
        console.log(`⚠️  User ${landlordData.email} already exists, skipping...`);
        return null;
      }
      throw authError;
    }
    ...
    console.error(`❌ Error creating landlord ${landlordData.email}:`, error);
    return null;
  }
};
...
const seedDatabase = async () => {
  ...
  for (const landlordData of TEST_LANDLORDS) {
    const landlordId = await createTestLandlord(landlordData);
    if (landlordId) {
      createdLandlords++;

      // Seed properties for this landlord
  ...
  console.log(`📊 Summary:`);
  console.log(`   👥 Landlords created: ${createdLandlords}`);
  console.log(`   🏠 Total properties: ${totalProperties}`);
  console.log(`   📈 Average properties per landlord: ${Math.round(totalProperties / createdLandlords)}`);
  console.log('');
...
if (require.main === module) {
  seedDatabase()
    .then(() => {
      console.log('✅ Comprehensive seeding script completed');
      process.exit(0);
    })
    .catch((error) => {
...
module.exports = { seedDatabase, createTestLandlord, generateProperty };
